...
void MCP7940RTC::clearControlBits()
{
	Wire.beginTransmission(MCP7940_CTRL_ID);
#if ARDUINO >= 100  
...
void MCP7940RTC::setBattEn()
{
	uint8_t rtcwkday;
	uint8_t rtcwkdayAddr = 0x03;
	Wire.beginTransmission(MCP7940_CTRL_ID);
#if ARDUINO >= 100
	Wire.write(rtcwkdayAddr);
#else
	Wire.send(rtcwkdayAddr);
#endif
	Wire.endTransmission();
	...
	rtcwkday = Wire.read();
#else
	rtcwkday = Wire.receive();
#endif

	rtcwkday |= 0x08; //Set VBATEN bit.
...
void MCP7940RTC::clearBattEn()
{
	...
	rtcwkday &= 0xf7; //Clear VBATEN bit.
...
void MCP7940RTC::setSquareWaveOutput()
{
	//Set SQW to be 1 Hz
	Wire.beginTransmission(MCP7940_CTRL_ID);
#if ARDUINO >= 100
	Wire.write((uint8_t)0x07); //Address of CONTROL register
	Wire.write((uint8_t)0x40); //Enable square wave output (4) and diable others; set the wave frequency == 1 Hz (0)
#else
	Wire.send(0x07);
	Wire.send(0x40);
#endif
...
void MCP7940RTC::clearSquareWaveOutput()
{
	clearControlBits();
}

void MCP7940RTC::setAlarm0(time_t t, uint8_t alarmMask)
{
	//Alarm Mask values ref in Table 5-5, in page 20 of the datasheet
	alarmMask |= 0x04; //set bit 3 (ALMPOL)
	alarmMask <<= 4;
	alarmMask |= 0x07; //ref page 12 of the datasheet
	tmElements_t tm;
	breakTime(t, tm);
	Wire.beginTransmission(MCP7940_CTRL_ID);
#if ARDUINO >= 100  
	...
	Wire.write((uint8_t)(dec2bcd(tm.Wday) | 0x70) & alarmMask);// wDay, trigger on minutes matching
	...
	Wire.send((dec2bcd(tm.Wday) | 0x70) & alarmMask);	// wDay
